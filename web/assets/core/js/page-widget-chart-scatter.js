...
rs.generateReport = function (data, years) {
	...
	$("#scatter-view").kendoChart({
		...
		series: [{
			...
			tooltip: {
				visible: true,
				template: breakdownTitle + " to " + netSalesTitle + " - #: dataItem.category # at #: dataItem.year #: #: kendo.toString(dataItem.valuePNLPercentage, 'n2') # % (#: kendo.toString(dataItem.valuePNL, 'n2') #)"
			}
		}],
		valueAxis: {
			majorGridLines: {
