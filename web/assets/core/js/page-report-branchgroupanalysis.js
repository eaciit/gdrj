...
ba.title = ko.observable('Branch Group Analysis');
ba.detail = ko.observableArray([]);
ba.limit = ko.observable(10);
ba.breakdownNote = ko.observable('');

ba.breakdownBy = ko.observable('customer.branchgroup');
...
ba.buildStructure = function (breakdownRD, expand, data) {
	...
	var keys = ["_id_customer_branchgroup", "_id_customer_channelid", "_id_customer_channelname"];
...
ba.render = function () {
	...
	var wrapper = toolkit.newEl('div').addClass('pivot-pnl-branch Group pivot-pnl').appendTo($('.breakdown-view'));
...
ba.optionBreakdownValues = ko.computed(function () {
	var branches = rpt.masterData.BranchGroup().map(function (d) {
		return { _id: d.Name, Name: d.Name };
	});
	return [ba.breakdownValueAll].concat(branches);
}, rpt.masterData.BranchGroup);
...
vm.breadcrumb([{ title: 'Godrej', href: viewModel.appName + 'page/landing' }, { title: 'Home', href: viewModel.appName + 'page/landing' }, { title: 'Branch Group Analysis', href: '#' }]);
