...
func main() {
	...
	resultSave := make(chan string, count)
	toolkit.Println("Saving")
	for wi:=1;wi<=10;wi++{
		go workerSave(wi, jobSave, resultSave)
	}
...
func workerAccum(wi int, jobs <-chan *gdrj.RawDataPL, result chan<- string) {
...
func workerSave(wi int, jobs <-chan *gdrj.PLDataModel, result chan<- string) {
	workerconn, _ := modules.GetDboxIConnection("db_godrej")
	defer workerconn.Close()

	for v := range jobs {
		e := workerconn.NewQuery().From(v.TableName()).Save().
			Exec(toolkit.M{}.Set("data",v))
		if e!=nil {
			toolkit.Println("Saving error: " + e.Error())
			os.Exit(100)
		}
		result <- "OK"
	}
