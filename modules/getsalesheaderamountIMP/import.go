...
func main() {
	...
	for {
		...
		if shs.Has(detail.GetString("SalesheaderID")) {
			sh := shs.Get(detail.GetString("SalesheaderID")).(*gdrj.SalesHeader)
			sh.SalesGrossAmount += detail.GetFloat64("SalesGrossAmount")
			sh.SalesNetAmount += detail.GetFloat64("SalesNetAmount")
			sh.SalesLine++
			shs.Set(detail.GetString("SalesheaderID"), sh)
		}
